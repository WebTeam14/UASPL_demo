...
import { dashboardStats, projects, drawings, inspections } from '@/data/mockData';
...
export default function DashboardHome() {
  ...
  const stats = user?.role ? dashboardStats[user.role as keyof typeof dashboardStats] : null;
...
<<<<<<< HEAD
      ...
      {/* Stats Grid - TMI View */}
      {user?.role === 'tmi' && stats && 'scheduledInspections' in stats && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            ...
            {drawings.map((drawing) => (
              <div key={drawing.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-foreground">{drawing.name}</p>
                    <p className="text-sm text-muted-foreground">
                      v{drawing.version} • {drawing.uploadedBy}
                    </p>
                  </div>
                </div>
        ...
        <Card className="enterprise-card">
          ...
          <CardContent className="space-y-4">
            {inspections.map((inspection) => (
              <div key={inspection.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                <div>
                  <p className="font-medium text-foreground">{inspection.type}</p>
                  <p className="text-sm text-muted-foreground">
                    {inspection.scheduledDate} • {inspection.inspector}
                  </p>
                </div>
